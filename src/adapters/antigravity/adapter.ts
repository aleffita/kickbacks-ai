...
// Status text patterns from the jetskiAgent bundle's thinking state machine
const ANCHORS = [
...
export class AntigravityAdapter implements TargetAdapter {
  ...
  /** Return app version from the parent package.json, or "jetski-agent". */
  version(): string | null {
    try {
  ...
  preflight(): PreflightResult {
    try {
      ...
      // Compatibility check: at least one anchor text must exist in the
      // bundle to confirm this is a jetskiAgent build.
  ...
  private ensureBackup(): Buffer | null {
    ...
    if (raw.indexOf(BLOCK_START) !== -1) {
      dlog("ext", "antigravity.backup.refused",
  ...
  applyPatch(p: PatchParams): OpResult {
    try {
      ...
      // Strip any stale block, then append the fresh one
